...
Фильтрация осуществляется путем добавления параметра `model_uid`. Она исключает все метрики и оставляет только метрики определенной модели. Данная фильтрация работает в формате JSON и Prometheus. Примеры:
